...
try:
    ...
    cur = conn.cursor()
    with open('./memo/public/dyn/sizelevel.csv', 'w') as outfile:
        ...
        for state in states:
            ...
            select_string = 'SELECT spb, size FROM feelevel NATURAL JOIN state WHERE statetime = '+ str(state)+" ORDER BY spb DESC"
    ...
    print "DBError in sizelevel_to_dygraph_csv.py: %s" % e.args[0]
    sys.exit(1)
