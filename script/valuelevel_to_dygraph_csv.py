...
try:
    ...
    cur = conn.cursor()
    with open('./memo/public/dyn/valuelevel.csv', 'w') as outfile:
        ...
        for state in states:
            ...
            select_string = 'SELECT spb, value FROM feelevel NATURAL JOIN state WHERE statetime = '+ str(state)+" ORDER BY spb DESC"
    ...
    print "DBError in valuelevel_to_dygraph_csv.py: %s" % e.args[0]
    sys.exit(1)
